...
const STORAGE_KEY = 'icf-acc-exam-state-v5';
...
function shuffleQuestionOptions(question) {
  ...
  const correctText = question.options[labels.indexOf(question.answer)];
  const shuffledOptions = shuffle(question.options);
  return {
    ...question,
    options: shuffledOptions,
    answer: labels[shuffledOptions.indexOf(correctText)],
  };
}
...
function App() {
  ...
  function startExam() {
    ...
    if (selectedBank === 'mixed') {
      ...
      questions = (randomizeOrder ? shuffle(bank.questions) : [...bank.questions]).map(shuffleQuestionOptions);
      title = bank.label;
    }
    ...
    return (
      <main className="page hero">
        <section className="hero-card">
          ...
          <h1>选择一组 ACC 模拟考试</h1>
          <p>目前使用六组 ACC 进阶题库。原始的 questions.js 已从程序入口完全移除，不会被载入或抽题。</p>

          <div className="metrics">
            <div><strong>6</strong><span>高难度题库</span></div>
          ...
          <fieldset className="bank-picker">
            ...
            <label className={`bank-option mixed ${selectedBank === 'mixed' ? 'selected' : ''}`}>
              ...
              />
              <Layers3 size={21} />
              <span><strong>混合随机卷</strong><small>从六组题库各随机抽取 10 题，再打乱组成全新的 60 题模拟卷。</small></span>
            </label>
          </fieldset>
